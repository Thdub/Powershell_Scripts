...
$DestinationPath = "$env:TEMP\DriversBackup"
   ...
   Write-Host "Creating"`"$DestinationPath`"
    ...
    Get-CIMInstance -ClassName MS_SystemInformation -NameSpace root\WMI 
        ...
        foreach($Driver in $ExportedDrivers) { 
            ...
            # Compress folder and remove
			  Compress-Archive "$DestinationPath\*" -CompressionLevel Fastest -DestinationPath "$env:USERPROFILE\Desktop\DriversBackup.zip" -Force
			     Remove-Item -Path $DestinationPath  -Recurse -Force
